...
import { useAppInfos, useAutoReloadOverlayBlocker } from '@strapi/helper-plugin';
import {
  Box,
  Loader,
  Alert,
...
const HomePage = () => {
  ...
  useEffect(async () => {
    ...
    await await Promise.all(
      ...
      [
        axios
          .get('/protected-populate/content-types')
          .then((response) => {
  ...
  const submitData = async () => {
    ...
    await axios
      .put('/protected-populate/data', selectedCheckboxes)
      .then(() => {
  ...
  return (
    <div>
      <Box background="neutral100">
        ...
        />
        {isVisible && (
          <ModalLayout onClose={() => handleSetIsVisible((prev) => !prev)} labelledBy="title">
            <ModalHeader>
            ...
            <ModalFooter
              startActions={
                <Button onClick={() => handleSetIsVisible((prev) => !prev)} variant="tertiary">
                  Cancel
                </Button>
              }
              endActions={
                <Button
                  disabled={modelContentType === '' || modelRoute === ''}
